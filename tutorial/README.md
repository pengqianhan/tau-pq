...
- [ ] **第 1 章** · 环境准备与冒烟测试（`uv sync` / `pytest` 全绿）
...
- [ ] 能用一句话分别说出 `tau_ai` / `tau_agent` / `tau_coding` 的职责
...
- [ ] `uv sync --dev` 成功，生成了 `.venv`
- [ ] `uv run tau --version` 输出 `tau 0.1.0`
- [ ] `uv run pytest -q` 全部通过
- [ ] `uv run pytest tests/test_agent_loop.py -q` 通过，并扫读了这个测试文件
- [ ] 完成后回到总览，把 **第 1 章** 那一行勾上
